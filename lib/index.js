...
const getCurrentConfigName = (allConfigFilenames, env) => {
...
const getConfigFilenames = (dirname, env) => {
  ...
  const subenvs = env.split('.');
  subenvs.some((subenv, i) => {
    const checkEnv = subenvs.slice(0, subenvs.length - i).join('.');
    currentConfigName = getCurrentConfigName(allConfigFilenames, checkEnv);
    return !!currentConfigName;
  });
    ...
    if (fileAccessible(namespaceDir)) {
      ...
      if (stat.isDirectory()) {
        ...
        defaultConfigNames.forEach(dcn => {
          ...
          if (fileAccessible(filename) && isFile(filename)) {
